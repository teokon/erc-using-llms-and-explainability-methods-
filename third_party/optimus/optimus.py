...
class Optimus:
    ...
    def __init__(self, model, tokenizer, label_names, task, set_of_instance=None):
        ...
        if self.set_of_instance is not None and len(self.set_of_instance) > 1:
    ...
    def __export_model_information__(self):
        ...
        if self.model_name.lower() == 'bert':
            # Read dims from the model config so this works for roberta-base
            # (12/12/768) AND roberta-large (24/16/1024), not just base.
            _cfg = self.model.model.config
            self.layers = _cfg.num_hidden_layers
            self.heads = _cfg.num_attention_heads
            self.embedding_size = _cfg.hidden_size
            self.ehe = _cfg.hidden_size // _cfg.num_attention_heads
    ...
    def explain(self, instance, mode='baseline', level='token', raw_attention='A'):
        ...
        if self.set_of_instance == None:
            # max_across is normally unavailable without a calibration set, BUT the winning
            # config may have been restored from a cached calibration (max_across_a set
            # externally) -- in that case the mode IS available and must not be rejected.
            _restored = getattr(self, 'max_across_a', None) not in (None, {'token': 0, 'sentence': 0})
            _allowed = ['baseline', 'max_per_instance', 'max_per_instance_per_label']
            if _restored:
                _allowed.append('max_across')
        ...
        # baseline returns interpretations[0] (config Mean,Mean,From), so only
        # that single config needs computing -- skip the other ~1.7k configs.
        # (max_per_instance still needs the full config sweep.)
        if mode == 'baseline':
            conf = conf[:1]
        # max_across (Optimus Batch): the winning config was already chosen during
        # calibration, so at explain time we only need to compute THAT one config.
        _across_shortcut = False
        if mode == 'max_across':
            idx = (self.max_across_a[level] if raw_attention == 'A'
                   else self.max_across_a_star[level])
            if isinstance(idx, str):   # 'Not Calculated' -> fall back to baseline config
                idx = 0
            conf = [conf[idx]]
            _across_shortcut = True
        ...
        elif mode == 'max_across' and _across_shortcut:
            return interpretations[0], tokens
    ...
    def __identify_max_across__(self):
        ...
        # Only token-level max_across is consumed downstream (we never call
        # sentence-level Batch), so calibrating just 'token' halves the (very
        # expensive) calibration cost. Set OPTIMUS_CALIB_LEVELS=token,sentence to
        # restore the original behaviour.
        _levels = os.environ.get("OPTIMUS_CALIB_LEVELS", "token").split(",")
        for level in _levels:
            ...
            # The A* calibration is a second full pass over the calibration set. It is
            # only needed if you later call explain(..., raw_attention='A*'); we always
            # use 'A', so it is skipped by default (set OPTIMUS_CALIB_ASTAR=1 to restore).
            if not os.environ.get("OPTIMUS_CALIB_ASTAR"):
