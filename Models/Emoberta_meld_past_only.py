#!/usr/bin/env python3
"""
MELD / RoBERTa-large — PAST-ONLY context ablation.

Same EmoBERTa-style pipeline as Emoberta_meld.ipynb (both-context baseline),
but the context window is built from PAST utterances only:

    <s>  [PAST context...]  </s>  TARGET  </s></s>

i.e. only utterances *before* the target are prepended (t-1, t-2, ...); no
future (right-side) utterances are appended. The two </s> still bracket the
target so the model knows which utterance to classify. This is the surgical
counterpart of the future-only script — context DIRECTION is the only variable.
...
OUTPUT_DIR = CHECKPOINTS_DIR / "emoberta_meld_large_past_only"
...
# =====================
# PAST-ONLY context builder
# =====================
# Identical to the baseline builder EXCEPT the right-side (future) expansion is
# removed: only past utterances (t-1, t-2, ...) are prepended into the window.
def build_context_dataset_past_only(df, tokenizer, max_length=512, speaker_caps=True, debug_n=3):
    df = df.copy()
    ...
    for d_id, g in df.groupby(DIALOG_COL, sort=False):
        ...
        for t in range(n):
            ...
            if dbg_printed < debug_n:
                print("=" * 80)
                print(f"DEBUG {dbg_printed+1} [PAST-ONLY] | dialog={d_id} | uttid={turns[t]} | label={labs[t]}")
...
train_ds_full = build_context_dataset_past_only(train_df, tok, max_length=MAX_LEN, debug_n=3)
val_ds_full   = build_context_dataset_past_only(val_df,   tok, max_length=MAX_LEN, debug_n=1)
test_ds_full  = build_context_dataset_past_only(test_df,  tok, max_length=MAX_LEN, debug_n=1)
print("Sizes:", len(train_ds_full), len(val_ds_full), len(test_ds_full))

save_constructed_csv(train_ds_full, OUTPUT_DIR / "train_constructed_past_only.csv", id2label=id2label)
save_constructed_csv(val_ds_full,   OUTPUT_DIR / "val_constructed_past_only.csv",   id2label=id2label)
save_constructed_csv(test_ds_full,  OUTPUT_DIR / "test_constructed_past_only.csv",  id2label=id2label)
...
for seed in SEEDS_FINAL:
    ...
    out_dir = str(OUTPUT_DIR / f"roberta_meld_past_only_seed{seed}")
...
repro_utils.save_repro_report(
    OUTPUT_DIR,
    config={
        "run_name": "MELD roberta-large PAST-ONLY",
