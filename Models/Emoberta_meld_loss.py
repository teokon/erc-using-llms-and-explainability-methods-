#!/usr/bin/env python3
"""
MELD / RoBERTa-large — BOTH-CONTEXT (past + future) baseline, whole dataset.

Executable version of Emoberta_meld.ipynb with all agreed changes:
  - roberta-large backbone
  - EmoBERTa-faithful </s></s> (double [SEP]) bracketing the target utterance
    (3-segment: past </s></s> current </s></s> future)
  - DGX raw-MELD paths, absolute OUTPUT_DIR, single-GPU pin
  - Optuna LR search (seeded) + final 5-seed training, per-epoch + BEST checkpoints
  - results_per_seed.csv / results_mean_std.csv
  - repro_report.{json,md} (preprocessing / context-length / truncation /
    hardware / runtime) via repro_utils.py

Run (inside tmux, single idle/assigned GPU set via CUDA_VISIBLE_DEVICES below):
    cd Models
    python Emoberta_meld.py \
        2>&1 | tee checkpoints/emoberta_meld_large_run.log
...
# Pin to a single GPU BEFORE importing torch (shared machine — set to your
# idle/assigned index; do NOT reuse a GPU another job is already using).
import sys, pathlib
...
# Raw MELD CSVs on the DGX (note the trailing space in the "Datasets " folder name)
_DATA = DATA_DIR / "Meld"
...
LOSS_TYPE = os.environ.get("LOSS_TYPE", "weighted_ce")   # weighted_ce | focal | class_balanced
assert LOSS_TYPE in ("weighted_ce", "focal", "class_balanced"), f"bad LOSS_TYPE={LOSS_TYPE}"
FIXED_LR = 2.05133826308745e-06   # reuse MELD baseline tuned LR (isolate the loss effect)
FOCAL_GAMMA = 2.0
CB_BETA = 0.999                   # class-balanced effective-number beta
OUTPUT_DIR = CHECKPOINTS_DIR / f"emoberta_meld_large_{LOSS_TYPE}"
...
def objective(trial):
    ...
    trainer = LossTrainer(
        loss_type=LOSS_TYPE, class_weights=CLASS_WEIGHTS, gamma=FOCAL_GAMMA,
        model=model,
        args=args,
    ...
    # MAXIMIZE validation weighted-F1 (NOT loss): focal/weighted loss values are
    # not monotonic with performance (focal loss is small for an undertrained
    # model that confidently over-predicts the majority class), so tuning on loss
    # selects an undertrained LR. Weighted-F1 is the metric we actually report.
    return out["eval_weighted_f1"]
...
# NOTE: Optuna LR search runs AFTER the loss setup below, USING the active loss.
# Each loss must tune its own LR because focal/weighted change the loss scale
# (reusing the plain-CE LR undertrains focal — its (1-p_t)^gamma factor shrinks
# the loss/gradients ~4x, so it needs a proportionally higher LR).
...
# =====================
# Imbalance loss setup: class weights + custom Trainer (weighted CE / focal)
# =====================
_train_labels = list(train_ds["labels"])
_counts = np.bincount(_train_labels, minlength=len(LABELS)).astype(float)
if LOSS_TYPE == "class_balanced":
    _eff = 1.0 - np.power(CB_BETA, _counts)          # Cui et al. 2019 effective number
    _w = (1.0 - CB_BETA) / np.clip(_eff, 1e-12, None)
else:                                                 # weighted_ce / focal -> balanced inverse-freq
    _w = compute_class_weight("balanced", classes=np.arange(len(LABELS)), y=_train_labels)
_w = _w / _w.mean()                                   # keep overall loss scale comparable
CLASS_WEIGHTS = torch.tensor(_w, dtype=torch.float, device=DEVICE)
print("[loss] type:", LOSS_TYPE, "| class weights:",
      {id2label[i]: round(float(x), 3) for i, x in enumerate(CLASS_WEIGHTS)})


class LossTrainer(Trainer):
    """Trainer with imbalance-aware loss. weighted_ce/class_balanced -> weighted CE;
    focal -> class-weighted focal loss (gamma=FOCAL_GAMMA)."""
    def __init__(self, *args, loss_type="weighted_ce", class_weights=None, gamma=2.0, **kwargs):
        super().__init__(*args, **kwargs)
        self._loss_type = loss_type; self._cw = class_weights; self._gamma = gamma

    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        labels = inputs.pop("labels")
        outputs = model(**inputs)
        logits = outputs.logits
        w = self._cw.to(logits.dtype) if self._cw is not None else None
        if self._loss_type == "focal":
            # Correct multiclass focal: p_t from the UNWEIGHTED CE, then apply the
            # class weight (alpha) as a SEPARATE per-example factor. (Putting the
            # weight inside the CE that feeds p_t distorts p_t = p^(w_t).)
            ce = nn.functional.cross_entropy(logits, labels, reduction="none")
            pt = torch.exp(-ce)
            focal = (1.0 - pt) ** self._gamma * ce
            if w is not None:
                focal = w[labels] * focal          # alpha_t per example
            loss = focal.mean()
        else:
...
# =====================
# Optuna LR search WITH the active loss -> per-loss tuned LR (resume-safe)
# =====================
...
if _bestlr_file.exists():
    ...
    print(f"[resume] reusing best_lr={best_lr:g} from {_bestlr_file} (skipping Optuna)")
else:
    ...
    print(f"[loss-ablation] LOSS_TYPE={LOSS_TYPE} | tuned LR={best_lr:g} "
          f"(Optuna, {N_TRIALS} trials, maximizing val weighted-F1)")
...
repro_utils.save_repro_report(
    OUTPUT_DIR,
    config={
        "run_name": f"MELD roberta-large BOTH-CONTEXT + {LOSS_TYPE}",
