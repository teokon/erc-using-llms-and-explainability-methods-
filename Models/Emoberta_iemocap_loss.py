#!/usr/bin/env python3
"""
IEMOCAP (6-way) / RoBERTa-large — CLASS-IMBALANCE LOSS ablation (both-context).
Select the loss via env var: LOSS_TYPE=weighted_ce|focal|class_balanced.
...
OUTPUT_DIR = CHECKPOINTS_DIR / f"emoberta_iemocap_large_{LOSS_TYPE}"
...
# =====================
# Imbalance-loss setup: class weights + custom Trainer (weighted CE / focal / class-balanced)
# =====================
_train_labels = list(train_ds_full["labels"])
...
class LossTrainer(Trainer):
    """weighted_ce/class_balanced -> weighted CE; focal -> class-weighted focal (gamma=FOCAL_GAMMA)."""
...
def objective(trial):
    ...
    trainer = LossTrainer(loss_type=LOSS_TYPE, class_weights=CLASS_WEIGHTS, gamma=FOCAL_GAMMA,
                      model=model, args=args, train_dataset=train_ds_full, eval_dataset=val_ds_full,
                      data_collator=collator, tokenizer=tok, compute_metrics=compute_metrics)
    trainer.train()
    # MAXIMIZE val weighted-F1 (loss is not monotonic with performance for focal/weighted).
    return trainer.evaluate(val_ds_full)["eval_weighted_f1"]
...
if _bestlr_file.exists():
    ...
    print("Best lr:", best_lr, "| Best val weighted-F1:", study.best_value)
...
repro_utils.save_repro_report(
    OUTPUT_DIR,
    config={
        "run_name": f"IEMOCAP roberta-large BOTH + {LOSS_TYPE}",
        "loss_type": LOSS_TYPE,
